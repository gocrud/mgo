...
func main() {
	...
	// 查询
	found, err := users.WithContext(ctx).FindByID(id)
	if err != nil {
		log.Fatal(err)
	}
	...
	// 更新
	err = users.Find().WithContext(ctx).ID(id).
		Set("name", "张三丰").
		Update()
	if err != nil {
		log.Fatal(err)
	...
	// 查询列表
	results, _ := users.Find().WithContext(ctx).
		OrderBy("created_at").
		Limit(10).
		All()
	...
	// 删除
	err = users.Find().WithContext(ctx).ID(id).Delete()
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println("删除成功")
